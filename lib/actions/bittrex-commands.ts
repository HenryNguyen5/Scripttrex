...
export async function sellFundsOnArrival({ currency, quantity, price }) {
    try {
        ...
        console.log('Funds have not yet arrived.... Polling balance again in five seconds');
        await sleep(5000)
...
async function sellFunds({ currency, quantity, price }){
    try {
        ...
        const response = await generatedApi.market.sellLimit({ market:market, quantity:quantity, rate:price });
        if (response.success === false) throw Error('Call to api failed')
